...
`final` y `const` declaran variables inmutables, pero con diferencias
importantes:
...
Un `StatefulWidget` es un widget que tiene estado mutable. Puede cambiar
su apariencia en respuesta a eventos.
...
El widget `Container` es un versátil contenedor que puede tener padding,
bordes, márgenes, color de fondo, etc.
...
persistente: - Tipos: `String`, `int`, `double`, `bool`, `List<String>`
- Almacenamiento: clave-valor - Uso ideal: preferencias,
