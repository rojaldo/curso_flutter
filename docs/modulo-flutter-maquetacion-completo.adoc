...
[arabic]
. link:#introducción-a-la-maquetación-en-flutter[Introducción a la
Maquetación en Flutter]
. link:#conceptos-fundamentales[Conceptos Fundamentales]
. link:#widgets-básicos-de-layout[Widgets Básicos de Layout]
. link:#filas-y-columnas[Filas y Columnas]
. link:#contenedores-y-decoración[Contenedores y Decoración]
. link:#material-design-en-flutter[Material Design en Flutter]
. link:#el-widget-scaffold[El Widget Scaffold]
. link:#componentes-material-esenciales[Componentes Material Esenciales]
. link:#widgets-de-desplazamiento[Widgets de Desplazamiento]
. link:#layouts-responsivos[Layouts Responsivos]
. link:#buenas-prácticas-de-maquetación[Buenas Prácticas de Maquetación]
...
La maquetación (o layout) es el proceso de organizar y posicionar
elementos visuales en una interfaz de usuario. En Flutter, todo es un
widget, y la maquetación se logra mediante la composición de widgets que
definen cómo se distribuyen los elementos en pantalla.

==== Filosofía de Flutter

Flutter sigue el principio de *"`Everything is a Widget`"*: - Los
elementos visuales son widgets (`+Text+`, `+Image+`, `+Icon+`) - Los
elementos de layout son widgets (`+Row+`, `+Column+`, `+Container+`) -
Los elementos estructurales son widgets (`+Scaffold+`, `+AppBar+`,
`+Drawer+`)
...
[arabic]
. *Padre pasa restricciones al hijo*: "`Puedes ser tan ancho como X y
tan alto como Y`"
. *Hijo decide su tamaño*: Dentro de esas restricciones, el hijo decide
. *Padre posiciona al hijo*: El padre coloca al hijo en su posición
final
...
Controla la distribución en el eje principal (horizontal en Row,
vertical en Column):
...
Controla la alineación en el eje transversal (vertical en Row,
horizontal en Column):
...
Material Design es un sistema de diseño creado por Google que se basa
en:

[arabic]
. *Material*: Superficies con elevación y sombras
. *Movimiento*: Animaciones significativas
. *Tipografía*: Escalas y estilos consistentes
. *Color*: Paletas coherentes y accesibles
. *Iconografía*: Iconos claros y consistentes
...
El Scaffold proporciona la estructura básica de una pantalla Material
Design:
...
*Causa:* Un widget Row o Column intenta ocupar más espacio del
disponible.
...
*Causa:* Usar widgets Material (Scaffold, AppBar, etc.) sin un ancestro
Material.
...
La maquetación en Flutter se basa en la composición de widgets y la
comprensión del sistema de restricciones. Los puntos clave son:

[arabic]
. *Widgets de Layout*: Container, Row, Column, Stack son los
fundamentales
. *Restricciones*: Entender cómo pasan de padre a hijo
. *Material Design*: Usar componentes Material para interfaces
consistentes
. *Responsive*: Adaptar layouts a diferentes tamaños de pantalla
. *Buenas prácticas*: Evitar nesting excesivo, usar const, separar
widgets
