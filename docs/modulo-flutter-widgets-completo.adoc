...
[arabic]
. link:#introducción-a-los-widgets[Introducción a los Widgets]
. link:#ciclo-de-vida-de-los-widgets[Ciclo de Vida de los Widgets]
. link:#statelesswidget[StatelessWidget]
. link:#statefulwidget[StatefulWidget]
. link:#comunicación-entre-widgets[Comunicación entre Widgets]
. link:#listeners-y-elementos-interactivos[Listeners y Elementos
Interactivos]
. link:#animaciones-de-widgets[Animaciones de Widgets]
. link:#estructura-de-una-aplicación-flutter[Estructura de una
Aplicación Flutter]
. link:#patrones-de-diseño-con-widgets[Patrones de Diseño con Widgets]
. link:#widgets-personalizados[Widgets Personalizados]
...
En Flutter, todo es un widget. Un widget es la unidad básica de
construcción de la interfaz de usuario que describe una parte de la UI.
Los widgets son inmutables: una vez creados, no pueden cambiar.
...
[arabic]
. *Widgets de Estado (Stateful/Stateless)*: Manejan datos y estado
. *Widgets de Layout*: Organizan otros widgets (Row, Column, Stack)
. *Widgets de Presentación*: Muestran contenido (Text, Image, Icon)
...
El StatelessWidget es simple: 1. Se crea el widget 2. Se llama a
`+build()+` para renderizar 3. Se destruye cuando ya no se necesita
...
Un `+StatelessWidget+` es un widget que no tiene estado mutable. Una vez
construido, su apariencia no cambia a menos que se proporcione nueva
información desde su padre.
...
Un `+StatefulWidget+` es un widget que tiene estado mutable. El estado
puede cambiar durante la vida del widget, y cuando cambia, el widget se
reconstruye.
...
Los widgets son el corazón de Flutter. Entender su ciclo de vida,
comunicación y patrones de diseño es esencial para construir
aplicaciones robustas:

[arabic]
. *StatelessWidget*: Para contenido estático
. *StatefulWidget*: Para estado mutable
. *setState()*: Para actualizaciones locales
. *Callbacks*: Para comunicación hijo → padre
. *Provider/BLoC*: Para estado global
. *Animaciones*: Implicit o con AnimationController
. *Estructura*: Organizar por capas y responsabilidades
